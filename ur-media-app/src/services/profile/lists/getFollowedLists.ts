...
export async function getFollowedLists(supabase: any, userId: string): Promise<ProfileTrackedMediaProps[]> {
  ...
  `)
    ...
    const animeMovieRows = rows.filter((row) => row.media?.media_type === "anime_movie");
    const animeShowRows = rows.filter((row) => row.media?.media_type === "anime_show");
    ...
    const animeIds = [...animeMovieRows, ...animeShowRows].map((row) => row.media_id);
    ...
    const [
        ...
        movieIds.length
            ? supabase
                ...
                .select("media_id, watch_status, starting_date, ending_date, rating, review, repeat_count")
                .eq("user_id", userId)
        ...
        showIds.length
            ? supabase
                ...
                .select("media_id, watch_status, starting_date, ending_date, rating, episodes_watched, review, repeat_count")
                .eq("user_id", userId)
                .in("media_id", showIds)
            : Promise.resolve({ data: [], error: null }),
        ...
        gameIds.length
            ? supabase
                ...
                .select("media_id, watch_status, starting_date, ending_date, rating, hours_played, review, repeat_count")
                .eq("user_id", userId)
                .in("media_id", gameIds)
            : Promise.resolve({ data: [], error: null }),
    ...
    const movieMap = new Map((trackedMoviesRes.data ?? []).map((item: any) => [item.media_id, item]));
    const showMap = new Map((trackedShowsRes.data ?? []).map((item: any) => [item.media_id, item]));
    const animeMap = new Map((trackedAnimeRes.data ?? []).map((item: any) => [item.media_id, item]));
    const gameMap = new Map((trackedGamesRes.data ?? []).map((item: any) => [item.media_id, item]));
    const bookMap = new Map((trackedBooksRes.data ?? []).map((item: any) => [item.media_id, item]));
        ...
        const items = await Promise.all(
            sourceRows.map(async (row) => {
                ...
                return {
                    ...
                    startingDate: tracked.starting_date,
                    endingDate: tracked.ending_date,
                    rating: tracked.rating,
                    episodesWatched: tracked.episodes_watched,
                    hoursPlayed: tracked.hours_played,
                    review: tracked.review,
                    repeatCount: tracked.repeat_count,
                    posterUrl,
                };
            })
        );

        return items.filter((item): item is ProfileTrackedMediaProps => item !== null);
    }

    const [movies, shows, animeMovies, animeShows, games, books] = await Promise.all([
