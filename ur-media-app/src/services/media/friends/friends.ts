  ...
  const friendInfo: FriendTrackedMedia[] = await Promise.all(
    friendIds.map(async ({ following_id }: { following_id: string }) => {
      ...
      const { data: animes, error: animeError } = await supabase
        ...
        .select(
          "watch_status, rating, episodes_watched, review, updated_at, media:media_id(source, media_type, external_id)",
        )
      ...
      const { data: books, error: bookError } = await supabase
        ...
        .select(
          "watch_status, rating, repeat_count, review, updated_at, media:media_id(source, media_type, external_id)",
        )
        .eq("user_id", following_id)
        .order("updated_at", { ascending: false })
        .limit(1);
      ...
      const { data: games, error: gameError } = await supabase
        ...
        .select(
          "watch_status, rating, hours_played, review, updated_at, media:media_id(source, media_type, external_id)",
        )
        .eq("user_id", following_id)
        .order("updated_at", { ascending: false })
        .limit(1);
      ...
      const mostRecent = allMedia.sort(
