...
export default function Friends({
  ...
  friendInfo: FriendTrackedMedia[];
  posterInfo: CountdownTitleAndUrl[];
}) {
  ...
  const validateForm = () => {
    if (newFriend === "" || newFriend.length > 16) {
            ...
            <div className={styles.updateCards}>
              {friendInfo.map((friend, index) => (
                <UpdateCard
                  key={friend.username}
                  updateInfo={friend}
                  mediaInfo={posterInfo[index]}
                />
              ))}
            </div>
          </>
        ) : (
          <p className={styles.emptyMessage}>Add friends to see recent activity.</p>
        )}
      </div>
    </div>
  );
